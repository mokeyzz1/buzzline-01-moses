...
def generate_messages():
    """
    MK custom generator: yields track & training style messages
    Includes occasional MK-ALERTs for consumer to detect.
    """
    athletes = ["Moses", "Teammate", "Coach", "System"]
    events = [
        "completed warmup",
        "started sprint set",
        "finished hurdle drill",
        "logged hydration",
        "entered recovery phase",
        "MK-ALERT: PR pace detected",
        "MK-ALERT: fatigue warning",
    ]

    while True:
        athlete = random.choice(athletes)
        event = random.choice(events)
        yield f"{athlete} {event}."
